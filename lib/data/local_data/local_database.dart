...
class LocaleDatabase {
  ...
  Future<void> initDatabase({String? initialTableName}) async {
    database = await openDatabase(
      ...
      onCreate: (db, version) {
        return db.execute(
          'CREATE TABLE ${initialTableName ?? "Items"}(id TEXT PRIMARY KEY, name TEXT, image TEXT, price DOUBLE, discount DOUBLE, discount_price INTEGER, count INTEGER, stock INTEGER, size TEXT, color TEXT)',
        );
      },
      version: 1,
    );
